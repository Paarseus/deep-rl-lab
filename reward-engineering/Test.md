...
| **Speed**          | Smooth Acceleration (Low Jerk)                | IMU sensor: compute `jerk = (a_t – a_prev)/dt`; if `|jerk| < threshold`                                      | **+2** per timestep                              |
| **Steering**       | Steady Steering (Low Steering Rate)           | `steering_rate = |(steer – steer_prev)/dt|` < 15°/s                                                         | **+1** per timestep                              |
